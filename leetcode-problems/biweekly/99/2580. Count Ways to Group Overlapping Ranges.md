...
[Leetcode](https://leetcode.com/contest/biweekly-contest-99/problems/count-ways-to-group-overlapping-ranges/)
<div>
...
![image](images/ways_to_group_overlapping_ranges.jpg)

```
 Ways_to_Group_Overlapping_Ranges(ranges) {
	n = len(ranges)
	ranges.sort() # sorting based on the starting value

	b = -1
	count = 0

	for s, e in ranges:
		# if start of the next range is after the maximum of 
		# previous ranges, then we can increase the new count of ranges
		if s >  b
			count += 1
		b  = max(b, e)
